...
scan_trivy() {  # $1 = target image, $2 = output path
  if command -v trivy >/dev/null 2>&1; then
    trivy image --format json --output "$2" "$1"
  else
    docker run --rm \
      -v /var/run/docker.sock:/var/run/docker.sock \
      -v "$PWD/out:/out" \
      "$TRIVY_IMAGE" image --format json --output "/out/$(basename "$2")" "$1"
  fi
}
...
for image in "${TARGETS[@]}"; do
  ...
  echo "==> triaging ${image}"
  patchtriage run "out/${safe}__trivy.json" "out/${safe}__grype.json" \
...
echo "Done. See benchmarks/out/BENCHMARKS.md"
